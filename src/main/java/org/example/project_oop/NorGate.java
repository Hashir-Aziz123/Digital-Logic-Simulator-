...
public class NorGate extends Gate
{
    ...
    NorGate(Connection input_line1 , Connection input_line2, Connection output_line)
    {
        // initialization
    ...
    public static void drawNorGate(MouseEvent event , AnchorPane placement_area)
    {
        //getting cords
        ...
        //adding event handling for input/output lines
        new NorGate(input_line1,input_line2,output_line);

        boolean canDraw = isCanDraw(placement_area, curved_line1, curved_line2, negates, input_line1, input_line2, output_line);

        // Add the gate components if no collision
        if (canDraw) {
            placement_area.getChildren().addAll(curved_line1 , curved_line2, negates, input_line1, input_line2, output_line);
        } else {
            System.out.println("Can not place nor gate on another nor gate");
        }

        // adding event handling for dragging the gate around
        curved_line1.setOnMousePressed( e -> { dragGate( curved_line1 , curved_line2 , negates , input_line1 ,input_line2 ,output_line , e , placement_area ); });
        curved_line2.setOnMousePressed( e -> { dragGate  ( curved_line1 , curved_line2, negates , input_line1 ,input_line2 ,output_line , e , placement_area ); });
        negates.setOnMousePressed( e -> { dragGate  ( curved_line1 , curved_line2, negates , input_line1 ,input_line2 ,output_line , e , placement_area ); });
        ...
        deleteButton.setOnAction(e -> {
            ...
            deleteGate(curved_line1 , curved_line2,negates  ,input_line1 , input_line2 , output_line, placement_area);
        });
    ...
    private static boolean isCanDraw(AnchorPane placement_area, QuadCurve curved_line1, QuadCurve curved_line2, Circle Negates, Connection input_line1, Connection input_line2, Connection output_line) {
        boolean canDraw = true;
        for (Node gate : placement_area.getChildren()) {
            if ( gate instanceof QuadCurve || gate instanceof Circle || gate instanceof Line || gate instanceof Rectangle)
            {
    ...
    public static void dragGate( QuadCurve curved_line1 , QuadCurve curved_line2 , Circle negates ,  Connection input_line1 ,
    ...
    public static void deleteGate(QuadCurve curved_line1, QuadCurve curved_line2 ,Circle negates ,  Connection input_line1, Connection input_line2 ,
                                  Connection output_line, AnchorPane placement_area)
    {
        ...
        placement_area.getChildren().removeAll( curved_line1 , curved_line2,negates , input_line1 , input_line2 , output_line);
